	...
	for _, tag := range []string{tagDirect, tagAPI} {
		t.Run(tag, func(t *testing.T) {
			_, err := GenerateEgressConfig(
				[]adapter.Outbound{{ID: 1, Tag: tag, Kind: "direct"}}, nil)
			if err == nil {
...
func TestOutboundsRequireTheirParams(t *testing.T) {
	for _, kind := range []string{"socks", "http", "wireguard"} {
		t.Run(kind, func(t *testing.T) {
			_, err := GenerateEgressConfig(
				[]adapter.Outbound{{ID: 1, Tag: "t", Kind: kind}}, nil)
			if err == nil {
...
func TestUnknownDefaultOutboundIsRefused(t *testing.T) {
	_, err := GenerateEgressConfig(nil, &adapter.Routing{DefaultOutboundTag: "missing"})
	if err == nil {
...
func TestSubjectRulesUseAccountingEmails(t *testing.T) {
	...
	if !strings.Contains(string(raw), subjectEmail(42)) {
		t.Errorf("rule does not use the accounting email for subject 42: %s", raw)
	}
}
