...
func BillableForSubject(
	...
	rows, err := st.Read().QueryContext(ctx, `
		WITH raw AS (
			SELECT node_id, service_id, outbound_id, uplink_bytes + downlink_bytes AS bytes
			  FROM usage_rollups_hourly
			 WHERE subject_id = ? AND hour_start >= ? AND hour_start < ?
			UNION ALL
			SELECT node_id, service_id, outbound_id, uplink_bytes + downlink_bytes
			  FROM usage_deltas
			 WHERE subject_id = ? AND created_at >= ? AND created_at < ?
			   AND id > COALESCE(
			       (SELECT last_delta_id FROM usage_rollup_state WHERE name = 'hourly'), 0)
		)
		SELECT raw.node_id, raw.service_id, raw.outbound_id, SUM(raw.bytes),
		       COALESCE(n.usage_coefficient, ?),
		       COALESCE(sv.usage_coefficient, ?),
		       COALESCE(o.usage_coefficient, ?)
		  FROM raw
		  LEFT JOIN nodes    n  ON n.id  = raw.node_id
		  LEFT JOIN services sv ON sv.id = raw.service_id
		  LEFT JOIN outbounds o ON o.id  = raw.outbound_id
		 GROUP BY COALESCE(raw.node_id, 0), COALESCE(raw.service_id, 0), COALESCE(raw.outbound_id, 0)
		 ORDER BY COALESCE(raw.node_id, 0), COALESCE(raw.service_id, 0), COALESCE(raw.outbound_id, 0)`,
		subjectID, from, to,
		subjectID, from, to,
		CoefficientUnit, CoefficientUnit, CoefficientUnit,
	)
	if err != nil {
	...
	for rows.Next() {
		var (
			nodeID, serviceID, outboundID sql.NullInt64
			bytes                         int64
			nodeCoef, svcCoef, outCoef    int64
		)
		if err := rows.Scan(&nodeID, &serviceID, &outboundID, &bytes, &nodeCoef, &svcCoef, &outCoef); err != nil {
